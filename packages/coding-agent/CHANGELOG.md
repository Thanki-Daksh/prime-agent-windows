...
- Added Layer 1 (Credential & Protected Path Shield) and Layer 2 (Destructive Command Interceptor & Environment Isolation).
- Added native Windows support for Python kernel bootstrap and Windows CLI launchers (prime-agent.cmd, prime-agent.ps1).
- Disabled automatic `~/.agents/skills` discovery by default behind `enableAgentsSkills` setting.
- Fixed flashing console windows on Windows by configuring background child process spawns with hidden windows.
