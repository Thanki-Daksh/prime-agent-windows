...
import { sanitizeKernelEnv } from "../security/security-policy.js";
...
export class KernelManager {
	...
	private async doStart(startOptions: KernelStartOptions): Promise<void> {
		...
		let forked = false;
		const baseEnv = this.options.env ? { ...process.env, ...this.options.env } : { ...process.env };
		const effectiveEnv = sanitizeKernelEnv(baseEnv);
