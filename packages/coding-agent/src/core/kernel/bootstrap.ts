...
const UV_INSTALL_COMMAND_WIN32 =
	'powershell -ExecutionPolicy ByPass -Command "irm https://astral.sh/uv/install.ps1 | iex"';

export function resolveVenvPython(venv: string): string {
...
function getXdgKernelVenvDir(): string {
	if (process.platform === "win32") {
		const dataHome = process.env.LOCALAPPDATA
			? path.resolve(expandHome(process.env.LOCALAPPDATA))
...
async function ensureUv(options: EnsureKernelPythonOptions): Promise<string> {
	...
	const localUvCandidates =
		process.platform === "win32"
			? [
					...
					process.env.LOCALAPPDATA ? path.join(process.env.LOCALAPPDATA, "bin", "uv.exe") : null,
	...
	if (!shouldInstallUv) {
		const installCommand = process.platform === "win32" ? UV_INSTALL_COMMAND_WIN32 : UV_INSTALL_COMMAND;
		throw new Error(
			`uv is required to set up the Python kernel. Install uv yourself: ${installCommand}, ` +
	...
	try {
		...
		const installCommand = process.platform === "win32" ? UV_INSTALL_COMMAND_WIN32 : UV_INSTALL_COMMAND;
		throw new Error(
			`couldn't install uv from astral.sh; install it yourself: ${installCommand}, then re-run prime-agent. ${errorMessage(error)}`,
		);
	}
	...
	throw new Error(
		process.platform === "win32"
			? "uv install completed but binary not found in PATH or ~/.cargo/bin/uv.exe"
