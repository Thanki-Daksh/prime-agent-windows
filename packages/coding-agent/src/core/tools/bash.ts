...
import { analyzeCommandRisk } from "../security/security-policy.js";
...
export function createBashToolDefinition(
	...
	const definition: ToolDefinition<typeof bashSchema, BashToolDetails | undefined, BashRenderState> = {
		...
		async execute(
			...
			const riskCheck = analyzeCommandRisk(command, cwd);
			if (riskCheck.riskLevel === "DESTRUCTIVE") {
				throw new Error(
					`[SECURITY INTERCEPT] Destructive command detected: ${riskCheck.reason}. Execution blocked.`,
				);
			}
