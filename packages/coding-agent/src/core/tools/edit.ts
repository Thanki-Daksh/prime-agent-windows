...
import { isProtectedPath } from "../security/security-policy.js";
import {
...
export function createEditToolDefinition(
	...
	const definition: ToolDefinition<typeof editSchema, EditToolDetails | undefined, EditRenderState> = {
		...
		async execute(_toolCallId, input: EditToolInput, signal?: AbortSignal, _onUpdate?, _ctx?) {
			...
			const pathCheck = isProtectedPath(absolutePath, cwd);
			if (pathCheck.protected) {
				throw new Error(`Access denied to protected credential path: ${path}. ${pathCheck.reason}`);
			}
